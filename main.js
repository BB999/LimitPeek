...
// 設定 trayWindow（'5h' | '7d'）で 5h 窓 / 7d 窓のどちらを出すか切り替え。
function trayItems(snap) {
  const items = [];
  const useSevenDay = settings && settings.trayWindow === '7d';
  const pick = (svc) => {
    if (!svc.enabled) return null;
    if (svc.error && !svc.data) return '…';
    const win = svc.data && (useSevenDay ? svc.data.sevenDay : svc.data.fiveHour);
